...
var prep_data = function(plot_info, nodes) {
  ...
  */
  ...
  var check_nodes = function(nodes) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      if (nodes[i].group in g.groups) {
        g.groups[nodes[i].group].push(nodes[i].name);
      }
      else {
        g.groups[nodes[i].group] = [nodes[i].name];
      }
  ...
  */
  ...
  var node_type = function(node) {
    ...
    if (node.source) {
      ...
      node.source.type = node.source.node.file;
    }
    if (node.target) {
      node.target.type = node.target.node.file;
  ...
  */
  ...
  //g.nodesByType.push({ key:     'source-target',                   values:  g.nodesByType[2].values });
